...
if(isset($_GET['delete'])){
    ...
    $sql = $db->query("DELETE FROM gallery_slider WHERE id= '$delete_id'") or die($db->error);

    if($sql)header("Location:gSliderTable.php");
} 
...
if(isset($_POST['create-post'])){
    ...
    $sql = "INSERT INTO gallery_slider(id,Slider_image,gallery_link) 
    VALUES 
    ('$id','$pix','$g_link')";
...
<div class="container-fluid">
    <h4 style = "display: flex; justify-content:center; align-item:center; font-size:45px; font-family: 'Times New Roman', Times, serif;">Gallery Slider</h4>
    <div class="row">
        <div class="container">
            <div class="row">
            <h6 style="font-family: 'Times New Roman', Times, serif;"><span style="color:blue">&nbsp; &nbsp; &nbsp; Gallery Slider </span>>> <span style="color:crimson;"> Gallery Slider </span>>>> Carousel </h6>
            </div>
        </div>
    </div>
        ...
        <div class="col-md-3">
            <form action="" method="post" enctype="multipart/form-data" class="was-validated">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="g_link" required>
                    <div class="valid-feedback">Valid.</div>
                    <div class="invalid-feedback">Please fill out this field.</div>
                </div>
            ...
            <form action="" method="post" enctype="multipart/form-data" class="was-validated">
                <?php 
                    ...
                    if(isset($_GET['Update_post'])){
                        ...
                        $sql = $db->query("SELECT * FROM gallery_slider where id = '$gid'" ) or die($db->error);
                        
                        if($row = $sql->fetch_assoc()){
                            $id_1 = $row['id'];
                            $pix1= $row['slider_image'];
                            $gallery_link1 = $row ['gallery_link'];
                        }
                    }

                    if(isset($_POST['Update_post'])){
                        $id = $_POST['pid'];
                        $g_link1 = $_POST['g_link'];
                        $pixx = $_FILES["pix"]["name"];
                        $target = "Htmlfolder/".$pix;
                        move_uploaded_file($_FILES["pix"]["tmp_name"],$target);
                    
                        $sql ="UPDATE gallery_slider SET slider_image = '$pixx',general_link='$g_link1' WHERE id = '$id' ";
                        
                        $result = $db->query($sql) or die($db->error);
                        if($result){
                            $msg1="Update successful";
                            echo "<script> window.location.href='gSliderTable.php?s=1'; </script>";
                            echo $msg1;
                        }
                    }
                ?>
                ...
                <div class="form-group">
                    <label for="text">id</label>
                    <input type="text" class="form-control" name="g_link" value="<?php if(isset($gallery_link1)) echo $gallery_link1;?>" required>
                    <div class="valid-feedback">Valid.</div>
                    <div class="invalid-feedback">Please fill out this field.</div>
                </div>
