...
if(isset($_GET['delete'])){
    ...
    if($sql)header("Location:multi_images.php");
} 
...
if(isset($_POST['create-post'])){
    ...
    $sql = "INSERT INTO multiple_ankara(id,photos,photo_link) 
    VALUES 
    ('$id','$pix','$c_link')";
            ...
            <form action="" method="post" enctype="multipart/form-data" class="was-validated">
                <?php 
                    ...
                    if(isset($_GET['Update_post'])){
                        ...
                        $sql = $db->query("SELECT * FROM multiple_ankara where id = '$gid'" ) or die($db->error);
                        
                        if($row = $sql->fetch_assoc()){
                            $id_1 = $row['id'];
                            $pix1= $row['photos'];
                            $photo_link1 = $row['photo_link'];
                        }
                    }
                    ...
                    if(isset($_POST['Update_post'])){
                        ...
                        $sql ="UPDATE multiple_ankara SET photos = '$pixx',photo_link = '$c_link1' WHERE id = '$id' ";
                        
                        $result = $db->query($sql) or die($db->error);
                        if($result){
                            $msg1="Update successful";
                            echo "<script> window.location.href='multi_images.php?s=1'; </script>";
                            echo $msg1;
                        }
                    }
                ?>
                ...
                <div class="form-group">
                    <label for="text">Page Link</label>
                    <input type="text" class="form-control" name="c_link" value="<?php if(isset($photo_link1)) echo $photo_link1;?>" required>
                    <div class="valid-feedback">Valid.</div>
                    <div class="invalid-feedback">Please fill out this field.</div>
                </div>
                ...
                <?php 
                    for($i=0; $i<$num; $i++){?>
                        <tr>
                            ...
                            <td> <a href="multi_images.php?Update_post=<?php echo $idsl[$i] ?>">
                                    <i class="fa fa-edit fa" style="margin-left: auto;color: black; font-size: 15px; margin-top: auto;"></i>
                                </a>
                            </td>
                            <td> <a href="multi_images.php?delete=<?php echo $idsl[$i] ?>">
